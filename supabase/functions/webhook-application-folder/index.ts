...
Deno.serve(async (req) => {
  ...
  const ctx = createRequestContext(req, "webhook-application-folder", clientIp);
  
  logRequestStart(ctx);

  const rateLimitResult = await checkRateLimit(supabase, clientIp, "webhook-application-folder", RATE_LIMIT_CONFIG);
